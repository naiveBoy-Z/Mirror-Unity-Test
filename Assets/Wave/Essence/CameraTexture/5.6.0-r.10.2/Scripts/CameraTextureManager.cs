...
namespace Wave.Essence.CameraTexture
{
	public class CameraTextureManager
	{
		...
		RenderThreadTask drawTextureTask = new RenderThreadTask(
			...
			(queue) => {
				lock (queue)
				{
					...
					bool enableCropping = msg.enableCropping;
					bool clearClampRegion = msg.clearClampRegion;

					bool updated = Interop.WVR_DrawTextureWithBuffer(nativeTexId, imgFormat, framebuffer, bufferSize, width, height, enableCropping, clearClampRegion);
		...
		private bool mStarted = false, startCameraCalled = false;
		...
		private bool enableCropping = true;
		private bool clearClampRegion = true;
		...
		{
			if (mStarted || startCameraCalled) return;
			syncPose = enable;
		...
		public bool isCroppingEnbled()
		{
			return enableCropping;
		}

		public bool isClampRegionCleared()
		{
			return clearClampRegion;
		}

		public void EnableCropping(bool enable)
		{
			enableCropping = enable;
		}

		public void ClearClampRegion (bool enable)
		{
			clearClampRegion = enable;
		}

		public void stopCamera()
		{
			if (!startCameraCalled) return; //Should only proceed when start camera has been called
		...
		private void drawTextureInRenderThread()
		{
			...
			lock (queue)
			{
				...
				msg.enableCropping = enableCropping;
				msg.clearClampRegion = clearClampRegion;
				if (syncPose)
				{
